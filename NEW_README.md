...
1. 增強實戰能力的「高級工具 (Advanced Tools)」
  ...
  1. 完善「自我意識」的動態更新 (Personality Consistency)
   * 建議：實作一個 update_personality 工具，讓大腦可以根據用戶的回饋（例如：「你說話太死板了」）主動修改 soul.md。
   * 價值：真正的自主生命體應該能根據環境調整自己的性格。

  2. 資料清理機制 (Data Housekeeping)
   * 建議：實作一個定期清理任務。
   * 細節：目前 temp_tts/ 目錄下的 .mp3 檔案會越積越多。我們需要一個機制在服務啟動時自動刪除超過 24 小時的臨時音訊，只保留 cache_tts/ 下的常用音訊。

  3. 加入「思考延遲模擬 (Natural Pause Simulation)」
   * 建議：目前的文字回傳太快，有時反而顯得像機器人。
   * 優化：在 on_status_update 中加入微小的隨機延遲，模擬大腦正在「組織語言」的動態感，配合語音會更自然。

  4. 實作「環境感知 (Environment Context)」
   * 建議：在大腦的 System Prompt 中，自動加入目前的操作系統、磁碟空間、或是 CPU 負載。
   * 價值：這樣當你問 「你現在過得好嗎？」，它能回答 「我現在磁碟快滿了，稍微有點沉重 (｡･ω･｡)」。
